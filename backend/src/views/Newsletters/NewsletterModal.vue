                        ...
                        <DialogPanel
                            ...
                            <header class="py-3 px-4 flex justify-between items-center">
                                <DialogTitle as="h3" class="text-lg leading-6 font-medium text-gray-900">
                                    {{ newsletter.id ? `Edit: ${props.newsletter.title}` : 'Add new Newsletter' }}
                                </DialogTitle>
                                <button
                                    @click="closeModal"
                            ...
                            <form @submit.prevent="onSubmit">
                                <div v-if="!loading" class="bg-white px-4 pt-5 pb-4">
                                    <CustomInput class="mb-2" v-model="newsletter.title" label="Title"/>
                                    <CustomInput class="mb-2" v-model="newsletter.content" label="Content"/>
                                </div>
...
const newsletter = ref({
    ...
    title: props.newsletter.question,
    content: props.newsletter.answer
})
...
function onSubmit() {
    ...
    if (newsletter.value.id) {
        store.dispatch('updateNewsletter', newsletter.value)
            .then(response => {
                loading.value = false;
        ...
        store.dispatch('createNewsletter', newsletter.value)
            .then(response => {
                loading.value = false;
