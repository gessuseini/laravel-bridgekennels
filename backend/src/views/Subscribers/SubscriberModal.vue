                        ...
                        <DialogPanel
                            ...
                            <header class="py-3 px-4 flex justify-between items-center">
                                <DialogTitle as="h3" class="text-lg leading-6 font-medium text-gray-900">
                                    {{ subscriber.id ? `Edit: ${props.subscriber.id}` : 'Add new Subscriber' }}
                                </DialogTitle>
                                <button
                                    @click="closeModal"
                            ...
                            <form @submit.prevent="onSubmit">
                                <div v-if="!loading" class="bg-white px-4 pt-5 pb-4">
                                    <CustomInput class="mb-2" v-model="subscriber.email" label="Email"/>
                                </div>
...
function onSubmit() {
    loading.value = true
    if (subscriber.value.id) {
        store.dispatch('updateSubscriber', subscriber.value)
            .then(response => {
                loading.value = false;
        ...
        store.dispatch('createSubscriber', subscriber.value)
            .then(response => {
                loading.value = false;
